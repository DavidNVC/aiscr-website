...
export default function ProgramsSection() {
  ...
  return (
    <section className="relative w-full border-b border-gray-100 overflow-hidden">
      ...
      {/* ── Row 1 : Stats ── */}
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-10 lg:px-16 py-12 sm:py-16">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-8 md:gap-8">
          {/* Left — Title */}
          <div className="shrink-0 md:w-70 lg:w-80">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold leading-tight text-gradient">
              {t("first_title")}
            </h2>
          </div>

          {/* Right — Stats */}
          <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-y-8 gap-x-6 w-full">
            {stats.map((stat) => (
              <div key={stat.key} className="flex flex-col gap-1">
                <span className="text-3xl sm:text-4xl md:text-5xl font-bold text-primary-blue">
                  {t(`stats.${stat.key}.value`)}
                </span>
                <span className="text-xs sm:text-sm leading-snug text-black-tertiary">
      ...
      {/* ── Row 2 : Programs ── */}
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-10 lg:px-16 py-12 sm:py-16 md:py-20">
        <div className="mb-8 sm:mb-12 text-center">
          <AppBadge className="mb-4 sm:mb-6">{t("badge")}</AppBadge>
          <h2
            className="text-2xl sm:text-3xl md:text-4xl font-bold leading-tight mb-3 sm:mb-4 text-gradient block"
            style={{ color: "#0a1636" }}
          >
            {t("title")}
          </h2>
